    ...
    if using_open_id?
      authenticate_with_open_id(params[:openid_url], :return_to => open_id_create_url, 
        ...
        if result.successful?
          create_new_user(:identity_url => identity_url, :login => registration['nickname'], :email => registration['email'], :name => registration['fullname'])
          Mailer.deliver_activation(user, request.host_with_port)
        else
      ...
      Mailer.deliver_signup_notification(@user, request.host_with_port)
    end
  end
